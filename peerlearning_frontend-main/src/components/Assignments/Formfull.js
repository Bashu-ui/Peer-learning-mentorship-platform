...
const Formfull = (prop) => {
  ...
  useEffect(() => {
    const fetchTeacherData = async () => {
      try {
        const response = await fetch(`${G_API}/courses/${prop.name.id}/teachers`, {
          method: "GET",
          headers: {
        ...
        if (data && data.teachers && data.teachers.length > 0) {
          ...
          if (lastTeacher?.profile?.name?.fullName) {
            setTeachersName(lastTeacher.profile.name.fullName);
          }
        }
      } catch (error) {
